...
class RevelStream(HttpStream, ABC):
    """
    ...
    `class RevelStream(HttpStream, ABC)` which is the current class
    `class Customers(RevelStream)` contains behavior to pull data for customers using v1/customers
    `class Employees(RevelStream)` contains behavior to pull data for employees using v1/employees

    If some streams implement incremental sync, it is typical to create another class
    `class IncrementalRevelStream((RevelStream), ABC)` then have concrete stream implementations extend it. An example
    is provided below.
...
class IncrementalRevelStream(RevelStream, ABC):
    """
...
# Source
class SourceRevel(AbstractSource):
